...
const ChatHeader: React.FC<ChatHeaderProps> = ({ title, onSearch }) => {
  ...
  const iconColor = isDark ? '#d1d5db' : '#333';
    ...
    <>
      <View className="bg-white dark:bg-secondary-dark px-4 py-3 border-b border-gray-200 dark:border-gray-700 pt-16">
        <View className="flex-row justify-between items-center">
          <Text className="text-xl font-bold text-gray-800 dark:text-foreground-dark">{title}</Text>
      ...
      {searchVisible && (
        <View className="bg-white dark:bg-secondary-dark px-4 py-2 border-b border-gray-200 dark:border-gray-700">
          <View className="flex-row items-center bg-gray-100 dark:bg-gray-700 rounded-lg px-3 py-2">
            <Ionicons name="search" size={20} color="#999" />
            <TextInput
              className="flex-1 ml-2 text-base text-gray-800 dark:text-foreground-dark"
      ...
      >
        ...
        >
          <View className="absolute right-4 top-28 bg-white dark:bg-gray-800 rounded-lg shadow-lg w-64 overflow-hidden">
            {menuItems.map((item, index) => (
              ...
              >
                <MaterialIcons name={item.icon as any} size={20} color={isDark ? '#d1d5db' : '#333'} />
                <Text className="ml-3 text-base text-gray-800 dark:text-foreground-dark">{item.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </TouchableOpacity>
