...
// Lấy thông tin tổng quan về ví của người dùng
export const getWalletOverview = createAsyncThunk(
  'wallet/getWalletOverview',
  async (_, { rejectWithValue }) => {
    try {
      return await http.get('/contract/wallet/balance');
    } catch (e: any) {
      return rejectWithValue(e?.message || 'Không thể tải thông tin ví');
    }
  }
);
...
export const getWalletTransactions = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      return await http.get(`/contract/wallet/transactions${qs ? `?${qs}` : ''}`);
    } catch (e: any) {
      return rejectWithValue(e?.message || 'Không thể tải lịch sử giao dịch ví');
    }
  }
);
...
export const confirmWalletTopup = createAsyncThunk(
  ...
  async (
    ...
    try {
      return await http.post(`/contract/wallet/topup/${payload.transactionId}/confirm`, {
        paidAmount: payload.paidAmount,
        transactionRef: payload.transactionRef,
      });
    } catch (e: any) {
      return rejectWithValue(e?.message || 'Xác nhận nạp tiền thất bại');
    }
  }
);
...
export const getTopupStatus = createAsyncThunk(
  ...
  async (transactionId: string, { rejectWithValue }) => {
    try {
      return await http.get(`/contract/wallet/topup/${transactionId}/status`);
    } catch (e: any) {
      return rejectWithValue(e?.message || 'Không thể kiểm tra trạng thái nạp tiền');
    }
  }
);
...
export const createWithdrawalRequest = createAsyncThunk(
  ...
  async (
    ...
    try {
      return await http.post('/contract/wallet/withdrawals', payload);
    } catch (e: any) {
      return rejectWithValue(e?.message || 'Không thể tạo yêu cầu rút tiền');
    }
  }
);
...
export const getWithdrawalRequests = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      if (params?.status) query.set('status', params.status);
      const qs = query.toString();
      return await http.get(`/contract/wallet/withdrawals${qs ? `?${qs}` : ''}`);
    } catch (e: any) {
      return rejectWithValue(e?.message || 'Không thể tải danh sách rút tiền');
    }
  }
);
